...
export default function CTAHistorySection() {
  ...
  useEffect(() => {
    getHomeContent()
      .then((data) => {
        if (data.sectionImages?.ctaHistory) {
          setImage(data.sectionImages.ctaHistory);
        }
      })
      .catch(() => {});
  }, []);
    ...
    >
      ...
      >
        <h2 className="font-heading text-xl md:text-2xl lg:text-3xl font-medium text-white leading-relaxed max-w-4xl mx-auto mb-8">
          With A Rich 50-Year History In Manufacturing High Quality Complex
          Compound Products, We&apos;re The Guys You Count On.
        </h2>
        <Link
          to="/about"
